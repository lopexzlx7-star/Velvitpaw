...
const GlassCard: React.FC<GlassCardProps> = ({ 
  ...
  useEffect(() => {
    if (videoRef.current) {
        videoRef.current.play().catch(() => {}); // Autoplay videos
    }
  }, [isHovered]);
  ...
  return (
    ...
    >
      <div 
        className="relative rounded-[2rem] overflow-hidden dark:bg-white/5 bg-gray-200/50 border dark:border-white/10 border-black/10 shadow-xl transition-all duration-500 group-hover:shadow-lg dark:group-hover:shadow-white/5 group-hover:-translate-y-1 cursor-pointer"
        onClick={handleClick}
      >
        <div className="relative overflow-hidden">
          {!isLoaded && (
            <div 
              className="absolute inset-0 dark:bg-white/5 bg-gray-300/20 animate-pulse flex items-center justify-center"
              style={{ height: item.height || 300 }}
            >
              <div className="w-10 h-10 rounded-full border-2 dark:border-white/10 border-black/10 dark:border-t-white/30 border-t-black/30 animate-spin" />
            </div>
          )}
              ...
              {isUserPost && onDelete && (
                 <button 
                    ...
                    className={`absolute top-4 right-4 p-2.5 backdrop-blur-xl rounded-2xl text-white transition-all z-20 ${
                        isConfirmingDelete ? 'bg-red-600/80 px-4' : 'bg-black/40 hover:bg-red-500/80'
                    }`}
                >
                    <Trash2 size={16} />
                </button>
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-between p-4">
            <div className="flex justify-end gap-2">
              {!isUserPost && onFollow && (
                <button 
                  onClick={(e) => { e.stopPropagation(); onFollow && onFollow(item.authorUid); }}
            ...
            <div className="flex items-center justify-between">
              ...
              <button onClick={(e) => e.stopPropagation()} className="p-2.5 bg-white/10 backdrop-blur-xl rounded-2xl text-white hover:bg-white/20 transition-all">
                <Share2 size={18} />
              </button>
            </div>
          </div>
      ...
      <div className="mt-3 px-2 flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <h3 className="text-[11px] font-black text-gray-900 dark:text-white uppercase tracking-wider truncate group-hover:opacity-70 transition-colors">
            {item.title}
          </h3>
        </div>
        <div className="flex items-center gap-2 text-gray-400 dark:text-white/20 flex-shrink-0">
          <div className="flex items-center gap-1">
            <Heart size={10} fill={isLiked ? "currentColor" : "none"} className={isLiked ? "text-red-500" : "text-inherit"} />

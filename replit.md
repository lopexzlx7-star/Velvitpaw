...
- **Media Storage**: Firestore inline base64 (imagens JPEG comprimidas) + Cloudinary (GIFs e vídeos via servidor)
...
- **Images**: Base64 JPEG comprimido (canvas, max 1200px, qualidade 0.82) salvo inline como `url` no documento do Firestore — sem upload externo.
- **GIFs**: Upload para Cloudinary via `/api/upload-image` para preservar a animação (bytes originais, sem canvas).
- **Videos**:
